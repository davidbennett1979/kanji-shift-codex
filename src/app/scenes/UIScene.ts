...
export class UIScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.topBar = this.add.rectangle(0, 0, width, 84, 0x130f0d, 0.88).setOrigin(0, 0).setScrollFactor(0).setDepth(1000);
    this.rightPanel = this.add.rectangle(width - 260, 84, 260, height - 84, 0x171310, 0.86)
      .setOrigin(0, 0)
      .setScrollFactor(0)
      .setDepth(1000);
    ...
    this.editorText = this.add.text(16, 56, '', {
      fontFamily: 'sans-serif',
      fontSize: '12px',
      color: '#c6d8d2',
      wordWrap: { width: Math.max(220, width - 300) },
    }).setDepth(1001);

    this.hintText = this.add.text(width - 248, 96, '', {
      fontFamily: 'sans-serif',
      fontSize: '13px',
      color: '#d8ceb8',
      wordWrap: { width: 232 },
    ...
    this.rulesText = this.add.text(width - 248, 170, 'Rules', {
      fontFamily: 'sans-serif',
      fontSize: '15px',
      color: '#f2e8d5',
      wordWrap: { width: 232 },
  ...
  updateState(state: HudState): void {
    ...
    this.editorText?.setText(
      editor?.enabled
        ? `EDITOR ON  |  [ / ] palette  |  Click place  |  Right-click erase  |  P export  |  Selected: ${editor.selectedGlyph} ${editor.selectedLabel}${editor.exportStatus ? `  |  ${editor.exportStatus}` : ''}`
        : ''
    );
  ...
  private onResize(gameSize: Phaser.Structs.Size): void {
    ...
    this.topBar?.setSize(width, 84);
    this.editorText?.setPosition(16, 56).setWordWrapWidth(Math.max(220, width - 300));
